...
exports.create = async(req, res) => {
    try {
        ...
        res.json(await new InAle(newData).save())
    } catch (err) {
        console.log(err);
...
exports.list = async(req, res) => {
    res.json(await InAle.find({}).sort({ createdAt: -1}).exec())
}
...
exports.remove = async(req, res) => {
    try {
        const deleted = await InAle.findOneAndDelete({ _id: req.params.id})
    ...
    exports.download = async (req, res) => {
        try {
          const file = await InAle.findOne({ _id: req.params.id});
          res.set({
