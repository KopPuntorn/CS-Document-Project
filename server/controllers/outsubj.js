...
exports.create = async(req, res) => {
    try {
        ...
        res.json(await new OutSubj(newData).save())
    } catch (err) {
        console.log(err);
...
exports.list = async(req, res) => {
    res.json(await OutSubj.find({}).sort({ createdAt: -1}).exec())
}
...
exports.remove = async(req, res) => {
    try {
        const deleted = await OutSubj.findOneAndDelete({ _id: req.params.id})
