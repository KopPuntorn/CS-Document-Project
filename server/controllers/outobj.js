...
exports.create = async(req, res) => {
    try {
        ...
        res.json(await new OutObj(newData).save())
    } catch (err) {
        console.log(err);
...
exports.list = async(req, res) => {
    res.json(await OutObj.find({}).sort({ createdAt: -1}).exec())
}
...
exports.remove = async(req, res) => {
    try {
        const deleted = await OutObj.findOneAndDelete({ _id: req.params.id})
    ...
    exports.download = async (req, res) => {
        try {
          const file = await OutObj.findOne({ _id: req.params.id});
          res.set({
